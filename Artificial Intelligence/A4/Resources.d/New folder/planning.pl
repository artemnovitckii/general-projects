...
satisfy([Goal|Rst_of_goals],State):-
    staticPreds(PredList),
    metaLevelPreds(MetaList),    
    functor(Goal, Name, Arity),
    (not(Name = not)) ->   %Check if is positive
     (member((Name/Arity), MetaList) ->  (call(Goal), satisfy(Rst_of_goals,State)); %Check if GoalPred is metalevel & directly executing its definition
     (member((Name/Arity), PredList) ->   (problem_initState(InitState), member(Goal,InitState), satisfy(Rst_of_goals,State));  
     (member(Goal, State), satisfy(Rst_of_goals,State))))
    ;
    (not(X)= Goal,
        not(satisfy(X,State)),
        satisfy(Rst_of_goals,State)).
...
fluentsOutFrom(List, OutList):-
    staticPreds(PredList),
    process(List, PredList,[], Final),
    reverse(Final,OutList).

process([H|T], PredList,NotEdgeCitiesList, FinalList):-
    (functor(H,Name, Arity),
     member((Name/Arity),PredList),
     process(T,PredList,NotEdgeCitiesList,FinalList));
    (functor(H,Name, Arity),
    not(member((Name/Arity),PredList)),
     process(T, PredList ,[H|NotEdgeCitiesList], FinalList)).

process([],PredList,NotEdgeCitiesList,FinalList):-
    NotEdgeCitiesList = FinalList.
