...
function searchGetShopItems(){
    const term = document.getElementById("shopInput").value;
    const searchurl = `http://redsox.uoa.auckland.ac.nz/ms/MuseumService.svc/shop?term=${term}`;
    const xhr1 = new XMLHttpRequest();
    xhr1.open("GET", searchurl, true);
    xhr1.setRequestHeader("Accept","Application/json")
    xhr1.onload = () => {
       const items = JSON.parse(xhr1.responseText);
       searchShowShopItems(items);
    }
    xhr1.send(null);
}


function searchShowShopItems(items){
    console.log(items);
    let shopItems = '';
    for (let i = 0; i < items.length; i++){
        const list1 = items[i];
        const description1 = list1.Description;
        const displayPicture =  '<img src="http://redsox.uoa.auckland.ac.nz/ms/MuseumService.svc/shopimg?id=' + list1.ItemId +'" width="50%" height="50%">';
        const title1 = list1.Title;
        const buylink = '<a href="http://redsox.uoa.auckland.ac.nz/mss/Service.svc/buy?id=' + list1.ItemId + '">Purchase</a>';
        let passPage =  '<h1 id="displayHeader">'+title1+"</h1>" + '<p id="newsParagraph">' + description1 + "</p>" + "</br></br>" + displayPicture + "</br></br>" + buylink + "</br></br>";
        if (i != items.length - 1){
            passPage += "<hr width=90% size=5px color=#bc6596 radius=5px>";
        }
        shopItems += passPage;
    }
    document.getElementById("ShopItems").innerHTML = shopItems;
}
...
function registerUser(){
    ...
    let userDetails = { Address : document.getElementById("userAddress").value, Name : userName, Password : document.getElementById("userPassword").value};
    if (userName.length != 0 && (document.getElementById("userPassword").value).length != 0 && (document.getElementById("userAddress").value).length != 0){
        const registerLink = "http://redsox.uoa.auckland.ac.nz/ms/MuseumService.svc/register";
        const xhr = new XMLHttpRequest();
        ...
        xhr.onreadystatechange = () =>{
            if (xhr.statusText == "OK") {
                document.getElementById("feedback").innerHTML = "User " + userName + " registered!";
            } 

            else{
                document.getElementById("feedback").innerHTML = "User " + userName + " was not registered";
            }
        }
    }
}
